...
| R4 | **Small-sample fairness.** The Chi-Square verdict is unreliable for tiny samples. | `is_null` may mislead for small `numbers`. | `DEFAULT_QUANTITY = 1000`; the home page and docs note that larger samples are more accurate ([solution.md](../solution.md) §8). |
