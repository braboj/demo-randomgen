...
class TestRandomGenParamNumbers:
...
@pytest.mark.parametrize('randomgen', versions, indirect=True)
class TestRandomGenParamProbabilities:
    ...
    def test_int_set(self, randomgen):
        ...
        with pytest.raises(RandomGenTypeError):
            randomgen.set_probabilities({0.2})
            randomgen.validate_probabilities()
...
@pytest.mark.parametrize('randomgen', versions, indirect=True)
class TestRandomGenDistribution:
...
@pytest.mark.parametrize('randomgen', versions, indirect=True)
class TestRandomGenPerformance:
